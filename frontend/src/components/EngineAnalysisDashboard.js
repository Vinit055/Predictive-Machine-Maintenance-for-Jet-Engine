...
import { ResponsiveContainer, CartesianGrid, XAxis, YAxis, Tooltip, ScatterChart, Scatter, LineChart, Line, Legend, BarChart, Bar } from 'recharts';
...
const EngineAnalysisDashboard = () => {
  ...
  const calculateSensorStats = (data) => {
    ...
    sensorColumns.forEach(sensor => {
      const values = data.map(row => row[sensor]).filter(val => val !== undefined);
      if (values.length > 0) {
          ...
          <div className="mx-[35px]">
            <HeatMap
              ...
              cellStyle={(background, value, min, max) => ({
                background: `rgba(0, 151, 255, ${1 - (max - value) / (max - min)})`,
                fontSize: "11px",
              })}
